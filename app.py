    ...
    with conn.session as s:
        ...
        s.execute(text("""
            CREATE TABLE IF NOT EXISTS sales_history (
                item_name TEXT, quantity_sold FLOAT, 
                snapshot_date DATE, PRIMARY KEY (item_name, snapshot_date)
            );
        """))
...
def sync_to_db(df, mode):
    ...
    with conn.session as s:
        for i, row in df.iterrows():
            if mode == "ledger":
                ...
                q = text("""INSERT INTO sales_history (item_name, quantity_sold, snapshot_date)
                            VALUES (:n, :q, :d) ON CONFLICT (item_name, snapshot_date) DO UPDATE SET quantity_sold=EXCLUDED.quantity_sold""")
        ...
        s.commit()
    stat.success(f"✅ {mode.capitalize()} data for {today} archived to Database.")

# --- 3. SIDEBAR (All Sync Buttons Active) ---
with st.sidebar:
...
if all([df_s is not None, df_wh is not None, df_sl is not None, df_b is not None]):
    ...
    with t1:
        ...
        # RESTORED: Product Deep Dive (Corrected Logic)
        st.divider()
        ...
        fig_cei = px.bar(top_10, x='customer_name', y='closing_balance', text=top_10['CEI'].apply(lambda x: f"{x:.1f}%"),
                         color='CEI', color_continuous_scale='RdYlGn', title="Collection Efficiency: Top 10 Debtors")
        ...
        with st.form("map_form"):
            ...
            if st.form_submit_button("Save Mapping"):
                with conn.session as s:
                    ...
                    s.commit()
                st.rerun()
        st.write("Live Database Mappings:")
        st.dataframe(df_map)
else:
    st.info("Upload all 4 files to see the Dashboard. Each file can be archived to the database as soon as it is uploaded.")
