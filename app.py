...
def init_db():
    with conn.session as s:
        ...
        # Customer Ledger with Snapshot Date
        s.execute(text("""
        ...
        # Bill Details with Snapshot Date
        s.execute(text("""
            CREATE TABLE IF NOT EXISTS bills_history (
                bill_id TEXT, vendor_name TEXT, bcy_balance FLOAT, 
                snapshot_date DATE, PRIMARY KEY (bill_id, snapshot_date)
            );
        """))
        
        # Inventory with Snapshot Date
        s.execute(text("""
            CREATE TABLE IF NOT EXISTS inventory_history (
                sku_title TEXT, qty FLOAT, value FLOAT, 
                snapshot_date DATE, PRIMARY KEY (sku_title, snapshot_date)
            );
        """))
        s.commit()
...
# --- 2. UNIVERSAL SYNC ENGINE (With Date Tagging) ---
        ...
        for i, row in df.iterrows():
            if mode == "ledger":
                q = text("""INSERT INTO customer_history (customer_id, customer_name, closing_balance, snapshot_date)
                            VALUES (:id, :n, :bal, :d) ON CONFLICT (customer_id, snapshot_date) DO UPDATE SET closing_balance=EXCLUDED.closing_balance""")
                ...
                q = text("""INSERT INTO bills_history (bill_id, vendor_name, bcy_balance, snapshot_date)
                            VALUES (:id, :v, :bal, :d) ON CONFLICT (bill_id, snapshot_date) DO UPDATE SET bcy_balance=EXCLUDED.bcy_balance""")
                ...
                q = text("""INSERT INTO inventory_history (sku_title, qty, value, snapshot_date)
                            VALUES (:t, :q, :v, :d) ON CONFLICT (sku_title, snapshot_date) DO UPDATE SET value=EXCLUDED.value, qty=EXCLUDED.qty""")
                p = {"t": row['title'], "q": row['Qty'], "v": row['Value'], "d": today}
            
            s.execute(q, p)
            if i % 100 == 0: prog.progress(int((i+1)/total_rows*100))
        s.commit()
    stat.success(f"✅ {mode.capitalize()} data for {today} saved to Database.")
...
with st.sidebar:
    ...
    st.divider()
    st.header("📂 Data Sync (DB Archive)")
    
    f_sum = st.file_uploader("1. Customer Summary", type="csv")
    df_s = pd.read_csv(f_sum) if f_sum else None
    if df_s is not None and st.button("Sync Ledger to DB"): sync_to_db(df_s, "ledger")
        
    f_wh = st.file_uploader("2. Warehouse Export", type="csv")
    df_w = pd.read_csv(f_wh) if f_wh else None
    if df_w is not None and st.button("Sync Inventory to DB"): sync_to_db(df_w, "inventory")
    ...
    if df_b is not None and st.button("Sync Bills to DB"): sync_to_db(df_b, "bills")
...
if all([df_s is not None, df_w is not None, df_sl is not None, df_b is not None]):
    ...
    with t1:
        ...
        fig_cei = px.bar(top_10, x='customer_name', y='closing_balance', 
                         ...
                         color='CEI', color_continuous_scale='RdYlGn', title="Top 10 AR Debtors")
    ...
    st.info("Please upload all 4 files in the sidebar to activate the Dashboard and Product Filter.")
