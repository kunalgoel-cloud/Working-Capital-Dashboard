...
if all([f_inv, f_bill, f_sales, f_wh]):
    ...
    # DIO Logic (Restored to accurate COGS mapping)
    wh_sum = df_wh.groupby('title').agg({'Qty':'sum', 'Value':'sum'}).reset_index()
    wh_sum['unit_cost'] = wh_sum['Value'] / wh_sum['Qty'].replace(0, 1)
    
    # We use 'left' join to identify what is unmapped, but 'inner' for COGS to keep DIO accurate
    ...
    with tab_dash:
        ...
        with c2:
            st.warning(f"**Inventory Priority:** Total stock value is **₹{wh_sum['Value'].sum():,.0f}**.")

        # PRODUCT FILTER (RESTORED)
        st.subheader("🔍 Filter by Product")
        selected_product = st.selectbox("Select Product to inspect stats:", ["All Products"] + sorted(wh_sum['title'].tolist()))
        if selected_product != "All Products":
            p_data = wh_sum[wh_sum['title'] == selected_product].iloc[0]
            st.info(f"**{selected_product}**: Stock Value: ₹{p_data['Value']:,.2f} | Quantity: {p_data['Qty']}")
    ...
    # --- TAB 2: INVENTORY AGEING (RESTORED) ---
    with tab_ageing:
        ...
        with col_pie:
            st.plotly_chart(px.pie(item_stats, values='Value', names='Ageing Bucket', hole=0.4, color_discrete_map={"0-30 Days (Fast)": "#2ecc71", "31-90 Days (Healthy)": "#3498db", "90+ Days (High Risk)": "#e74c3c"}), use_container_width=True)
        with col_list:
    ...
    # --- TAB 3: MAPPINGS (FIXED MANUAL EDITOR) ---
    with tab_map:
        st.header("🔧 Mapping Management")
        
        # MANUAL EDITOR FORM (RESTORED)
